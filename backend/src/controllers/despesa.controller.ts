...
export const obterDespesa = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const despesa = await despesaService.obterDespesa(Number(id));
...
export const atualizarDespesa = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const despesa = await despesaService.atualizarDespesa(Number(id), req.body);
    res.json(despesa);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
export const removerDespesa = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    await despesaService.removerDespesa(Number(id));
