...
export const get = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const turma = await turmaService.getTurma(Number(id));
...
export const update = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const turma = await turmaService.updateTurma(Number(id), req.body);
    res.json(turma);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
export const remove = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    await turmaService.deleteTurma(Number(id));
