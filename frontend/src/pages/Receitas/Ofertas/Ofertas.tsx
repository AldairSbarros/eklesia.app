...
export default function Ofertas() {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/offerings?type=oferta`)
  ...
  return (
    <div className="ofertas-page">
      <h2>Ofertas</h2>
      <form className="ofertas-form" onSubmit={handleSubmit}>
      ...
      {mensagem && <div className="ofertas-msg">{mensagem}</div>}
      {loading ? (
        <p>Carregando...</p>
      ) : (
        <table>
          ...
          <tbody>
            {ofertas.map(o => (
              <tr key={o.id}>
                <td>{o.Member?.nome || o.memberId}</td>
                ...
                <td>
                  {o.receiptPhoto ? (
                    <a href={o.receiptPhoto} target="_blank" rel="noopener noreferrer">Ver</a>
                  ) : '-'}
                </td>
              </tr>
            ))}
